// src/App.jsx — v2.3 final: consumes isComputing from useCalendar
import { lazy, Suspense, memo, useDeferredValue, useCallback } from 'react';
import { useBrand }    from './hooks/useBrand.js';
import { useCalendar } from './hooks/useCalendar.js';
import { useExport }   from './hooks/useExport.js';
import { useSteps }    from './hooks/useSteps.js';

const BrandForm        = lazy(() => import('./components/BrandForm.jsx'));
const PlatformSelector = lazy(() => import('./components/PlatformSelector.jsx'));
const CalendarResults  = lazy(() => import('./components/CalendarResults.jsx'));

const preloadStep2 = () => import('./components/PlatformSelector.jsx');
const preloadStep3 = () => import('./components/CalendarResults.jsx');

const StepIndicator = memo(({ step, isPending }) => (
  <div className="flex justify-center mb-6 relative">
    <div className="flex items-center gap-2">
      {['Brand Info', 'Platforms', 'Calendar'].map((_, idx) => {
        const n = idx + 1;
        return (
          <span key={n} className="flex items-center gap-2">
            <div className={[
              'w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all',
              step >= n ? 'bg-purple-500 text-white' : 'bg-slate-700 text-slate-400',
              step === n && isPending ? 'animate-pulse ring-2 ring-purple-400 ring-offset-2 ring-offset-slate-900' : '',
            ].join(' ')}>{n}</div>
            {n < 3 && <div className={`w-12 h-1 rounded transition-colors ${step > n ? 'bg-purple-500' : 'bg-slate-700'}`}/>}
          </span>
        );
      })}
    </div>
    {isPending && (
      <p className="absolute -bottom-5 text-xs text-purple-400 animate-pulse">
        ● Computing calendar…
      </p>
    )}
  </div>
));
StepIndicator.displayName = 'StepIndicator';

const StepSkeleton = () => (
  <div className="space-y-3 animate-pulse">
    {[1,2,3,4].map(i => <div key={i} className="h-12 bg-slate-800/80 rounded-xl"/>)}
    <div className="h-10 bg-slate-700/40 rounded-xl w-1/4 ml-auto"/>
  </div>
);

export default function App() {
  const { brand, updateBrand, toggleGoal, togglePlatform, resetBrand }     = useBrand();
  const { step, showCalendar, isPending, nextStep, prevStep, generate, reset } = useSteps();

  const deferredBrand = useDeferredValue(brand);
  const isStale       = brand !== deferredBrand;

  const { platformCalendars, isComputing } = useCalendar(deferredBrand, showCalendar);
  const { copied, building, handleDownload, handleCopy } = useExport(deferredBrand, platformCalendars, showCalendar);

  const handleReset = useCallback(() => { reset(); resetBrand(); }, [reset, resetBrand]);

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4">
      <div className="max-w-5xl mx-auto relative">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold text-white mb-1">Social Media Content Calendar</h1>
          <p className="text-purple-200 text-sm">v2.3 — Concurrent Mode Complete</p>
        </div>

        <StepIndicator step={step} isPending={isPending || isComputing} />

        <div className="mt-8">
          <Suspense fallback={<StepSkeleton />}>
            {step === 1 && (
              <BrandForm brand={brand} updateBrand={updateBrand} toggleGoal={toggleGoal}
                onNext={nextStep} onNextHover={preloadStep2} />
            )}
            {step === 2 && (
              <PlatformSelector selected={brand.platforms} onToggle={togglePlatform}
                onBack={prevStep} onGenerate={generate} onGenerateHover={preloadStep3} />
            )}
            {step === 3 && showCalendar && (
              <div className={`transition-opacity duration-300 ${isStale || isComputing ? 'opacity-50' : 'opacity-100'}`}>
                <CalendarResults
                  brand={deferredBrand}
                  platformCalendars={platformCalendars}
                  copied={copied}
                  building={building}
                  onDownload={handleDownload}
                  onCopy={handleCopy}
                  onReset={handleReset}
                />
              </div>
            )}
          </Suspense>
        </div>
      </div>
    </div>
  );
}
