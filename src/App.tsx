...
const StepIndicator = memo(({ step, isPending }: StepIndicatorProps) => (
  <div className="flex justify-center mb-8 relative">
    ...
    {isPending && (
      <p className="absolute -bottom-5 text-xs text-purple-400 animate-pulse" role="status" aria-live="polite">
        ● Computing calendar…
      </p>
    )}
  </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-4">
      <div className="max-w-5xl mx-auto relative">
        <header className="text-center mb-6">
          ...
          <p className="text-purple-200 text-sm">v3.0 — TypeScript + Accessibility</p>
        </header>
            ...
            {step === 3 && showCalendar && (
              <div className={`transition-opacity duration-300 ${isStale||isComputing?'opacity-50':'opacity-100'}`}
                ...
                <CalendarResults brand={deferredBrand} platformCalendars={platformCalendars}
                  copied={copied} building={building} onDownload={handleDownload} onCopy={handleCopy} onReset={handleReset}/>
              </div>
            )}
          </Suspense>
        </main>
