...
export default function PlatformSelector({ selected, onToggle, onBack, onGenerate, onGenerateHover }: PlatformSelectorProps) {
  return (
    <div className="bg-slate-800/80 rounded-xl p-5 backdrop-blur">
      ...
      <p className="text-slate-400 text-xs mb-4" aria-live="polite">{selected.length} of {ALL_PLATFORMS.length} selected</p>
        ...
        {ALL_PLATFORMS.map(p => {
          ...
          return (
            <button key={p} onClick={()=>onToggle(p as PlatformName)}
              ...
              className={`p-3 rounded-lg border-2 text-sm font-medium transition-all ${active?'text-white':'border-slate-600 bg-slate-700/50 text-slate-300 hover:border-slate-500'}`}
      ...
      <div className="mt-4 flex justify-between">
        <button onClick={onBack} aria-label="Go back to brand information"
          className="bg-slate-700 hover:bg-slate-600 text-white px-5 py-2 rounded-lg text-sm transition-colors">Back</button>
